...
class FilterBitsBuilder {
 public:
  ...
  // Returns a FilterBitsConfig for filters generated by this builder, only if
  // that interface should be used for reading the filters instead of a
  // FilterBitsReader. (In the latter case, returns nullptr.)
  //
  // FilterBitsConfig is a newer interface that is more efficient for newer
  // built-in Bloom(-like) filter types. FilterBitsReader is an older interface
  // used when metadata about the filter configuration is embedded in the
  // filter bits, or when the user's FilterPolicy uses a custom
  // FilterBitsReader/Builder.
  virtual std::shared_ptr<const FilterBitsConfig> GetConfig() const {
    // default impl; only built-in code should override
    return nullptr;
  }
...
// It should be initialized by a Slice generated by FilterBitsBuilder.
// NB: This can be an inefficient interface because it doesn't allow
// remembering the configuration/metadata of a filter without also pinning
// the bits of that filter in memory. Thus, either a Reader has to be
// tracked along with the underly filter memory, or a new Reader has to be
// generated, including re-reading metadata, each time the filter is accessed.
// FilterPolicy::MayMatch arguably provides a more friendly interface for
// interacting with block caching, but does not really accommodate difference
// in config used to generate a serialized filter and the current config
// setting for building new filters. Newer interface is FilterBitsConfig.
...
// TODO
enum class BuiltinFilterImpl {
  // Start at 2 to minimize confusion with boolean
  // Fast local Bloom is TODO
  kFastLocalBloom = 2,
};
...
// Like the other overload, but selects a newer built-in filter
// implementation as preferred (with full or partitioned filter).
// bits_per_key also relaxed to non-integers.
extern const FilterPolicy* NewBloomFilterPolicy(
    double bits_per_key, BuiltinFilterImpl preferred_impl);
