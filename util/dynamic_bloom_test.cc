...
TEST_F(DynamicBloomTest, perf) {
  ...
  for (uint32_t m = 1; m <= 8; ++m) {
    ...
    fprintf(stderr, "standard bloom, avg add latency %3g\n",
    ...
    assert(count > 0);
    fprintf(stderr, "standard bloom, avg query latency %3g\n",
            static_cast<double>(elapsed) / num_keys);
    ...
    elapsed = timer.ElapsedNanos();
    fprintf(stderr,
            "dynamic bloom, avg add latency %3g\n",
            static_cast<double>(elapsed) / num_keys);
    ...
    assert(count > 0);
    fprintf(stderr,
            "dynamic bloom, avg query latency %3g\n",
            static_cast<double>(elapsed) / num_keys);
...
TEST_F(DynamicBloomTest, concurrent_with_perf) {
  ...
  uint32_t locality_limit = 1;

  uint32_t num_threads = 4;
  std::vector<port::Thread> threads;

  for (uint32_t m = 1; m <= m_limit; ++m) {
    for (uint32_t locality = 1; locality <= locality_limit; ++locality) {
      Arena arena;
      const uint32_t num_keys = m * 8 * 1024 * 1024;
