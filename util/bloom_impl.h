...
// (Compare to LegacyLocalityBloomImpl<false> at 1.138%)
//
...
//
// Using only 32 bits within each cache line has negligible impact for any
// reasonable cache line / bucket size, for arbitrary filter size, and
// potentially saves intermediate data size in some cases vs. tracking full
// 64 bits. (Even in an implementation using 64-bit arithmetic to generate
// indices, I might do the same, as a single multiplication suffices to
// generate a pseudorandom 64 bits from 32 bits.)
//
