...
RfcUuid RfcUuid::FromRawLoseData(const RawUuid &raw) {
  ...
  u.data = (raw & (Unsigned128{0xffffffffffff} << 80)) |
           ...
           ((raw & (Unsigned128{0x3fffffffffffffff} << 6)) >> 6);
...
Unsigned128 RocksMuid::ReducedToScaled(const Unsigned128 &reduced) {
  ...
  // multiply by 2**128 / 36**20 ~= 25455957.05553etc. and round up. We
  ...
  static const Unsigned128 kReciprocal =
      (Unsigned128{0x1846d550e37} << 64) + 0xb5063df43be64e08U;
  ...
  // Find the right place for our decimal point and round up.
  // (208 + 48 == 256)
  return ((upper128 << 48) | (lower128 >> 80)) + ((lower128 << 48) != 0);
}
...
// The next prime number after the 40-bit golden ratio
constexpr uint64_t kGoldenPrime40 = 0x9e3779b99dU;
// The next prime number after the 44-bit golden ratio
constexpr uint64_t kGoldenPrime44 = 0x9e3779b9839;

RawUuid AsTop44Bits(uint64_t v) {
  return RawUuid{v & ((uint64_t{1} << 44) - 1)} << (128 - 44);
}

RawUuid AsMiddle40Bits(uint64_t v) {
  return RawUuid{v & ((uint64_t{1} << 40) - 1)} << 44;
}

RawUuid AsBottom44Bits(uint64_t v) {
  return RawUuid{v & ((uint64_t{1} << 44) - 1)};
}

}  // namespace

RawUuid XorInTopOf3Counter(const RawUuid &u, uint64_t counter) {
  return u ^ AsTop44Bits(counter) ^
         AsBottom44Bits((counter >> 44) * kGoldenPrime44);
}
RawUuid XorInMiddleOf3Counter(const RawUuid &u, uint64_t counter) {
  return u ^ AsMiddle40Bits(counter) ^
         AsTop44Bits((counter >> 40) * kGoldenPrime44);
}
RawUuid XorInBottomOf3Counter(const RawUuid &u, uint64_t counter) {
  return u ^ AsBottom44Bits(counter) ^
         AsMiddle40Bits((counter >> 44) * kGoldenPrime40);
}

RawUuid XorInNonMiddleCounter(const RawUuid &u, uint64_t counter) {
  return u ^ AsBottom44Bits(counter) ^ AsTop44Bits(counter >> 44);
}
