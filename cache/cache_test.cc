...
const std::string kFastLRU = "fast_lru";
const std::string kHyperClock = "hyper_clock";
const std::string kFastClock = "fast_clock";
...
class CacheTest : public testing::TestWithParam<std::string> {
 public:
  ...
  static void Deleter(const Slice& key, void* v) {
    if (type_ == kHyperClock || type_ == kFastClock) {
  ...
  std::shared_ptr<Cache> NewCache(size_t capacity) {
    ...
    if (type == kFastClock) {
      FastClockCacheOptions fcco(capacity);
      fcco.min_avg_entry_charge =
          std::min(estimated_value_size_, fcco.min_avg_entry_charge);
  ...
  std::shared_ptr<Cache> NewCache(
    ...
    if (type == kFastClock) {
      return FastClockCacheOptions(capacity, num_shard_bits,
                                   strict_capacity_limit, nullptr /*allocator*/,
                                   charge_policy, 1 /*min_avg_value_size*/)
          .MakeSharedCache();
    }
    return nullptr;
  }
  ...
  std::string EncodeKey(int k) {
    auto type = GetParam();
    if (type == kHyperClock || type == kFastClock) {
...
TEST_P(CacheTest, DefaultShardBits) {
  ...
  size_t min_shard_size =
      ((type_ == kHyperClock || type_ == kFastClock) ? 32U * 1024U : 512U) *
      1024U;
...
INSTANTIATE_TEST_CASE_P(CacheTestInstance, CacheTest,
                        testing::Values(kLRU, kHyperClock, kFastClock));
