...
std::atomic<uint64_t> generateRawUuidCounter{0};

}  // namespace

RawUuid GenerateRandomRawUuidExpensive(Env* env,
                                       bool exclude_track1_for_testing,
                                       bool exclude_track2_for_testing,
                                       bool exclude_track3_for_testing) {
  ...
  struct Init {
    ...
    // TODO? process ID
    // And pointer to the counter we will use, in case we have multiple
    // copies of the library loaded into memory
    uint64_t thread_id = 0;
    std::array<char, 64> hostname_buf{};
    int64_t time = 0;
    uint64_t nano_time = 0;
    uintptr_t counter_ptr = 0;
  ...
  if (!exclude_track1_for_testing) {
    ...
    init.counter_ptr = reinterpret_cast<uintptr_t>(&generateRawUuidCounter);
  }

  // Track 2: random_device
  if (!exclude_track2_for_testing) {
    std::random_device r;
...
RawUuid GenerateRawUuid() {
  uint64_t counter = ++generateRawUuidCounter;
  return XorInMiddleOf3Counter(GetRawUuidForCurrentProcess(), counter);
}
